...
impl Source {
    pub fn from_file(path: &Path) -> io::Result<Source> {
        ...
        Ok(Source::from_text(&name.to_string_lossy(), s))
    }

    pub fn from_text(name: &str, text: String) -> Source {
    ...
    // source(name: &str, text: &str) -> Source {
